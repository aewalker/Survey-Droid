...
{	
	...
	@ConfigKey(value="0", global=true)
	public static final String TIMES_SENT = TAG +"_times_sent"; //boot receiver resets this
	
	/**
	 * Use one of these for the status code in location updates
	 */
	public enum LocationCode
	{
		/** Indicates that everything is normal */
		OK,
		/** Indicates that times aren't being tracked now */
		BAD_TIME,
		/** Indicates that tracking is off */
		TRACKING_OFF,
		/** Indicates that we don't have a valid time now */
		NO_DATA,
		/** Indicates that the location is out of range */
		OUT_OF_RANGE;
	}
	...
	public void onReceive(Context ctxt, Intent intent)
	{
        ...
        Util.i(null, TAG, msg);
        if (!Config.getBoolean(ctxt, TIMES_COALESCED)) coalesceTimes(ctxt);
        sendLocation(ctxt);
	}
	...
	private void sendLocation(Context c)
	{
		...
		if (!Config.getBoolean(c, Config.TRACKING_LOCAL) ||
			!Config.getBoolean(c, Config.TRACKING_STUDY, true, 0)) //FIXME
		{
			Util.d(null, TAG, "Tracking is disabled; sending null location");
			TrackingDBHandler tdbh = new TrackingDBHandler(c);
			tdbh.writeLocation(LocationCode.TRACKING_OFF, Util.currentTimeAdjusted() / 1000);
			uploadNow(c);
			return;
		}
		...
		boolean log = false;
		int numTimes = 0; //Config.getSetting(c, Config.NUM_TIMES_TRACKED, 0); //FIXME
		if (numTimes == 0)
		{
		...
		else
		{
			...
			for (int i = 0; i < numTimes; i ++)
			{
				...
				{
					//FIXME
					start = 0; //Integer.parseInt(Config.getSetting(
							//c, Config.TRACKED_START + i, null));
					end = 0; //Integer.parseInt(Config.getSetting(
		...
		if (log == false)
		{
			...
			TrackingDBHandler tdbh = new TrackingDBHandler(c);
			tdbh.writeLocation(LocationCode.BAD_TIME, Util.currentTimeAdjusted() / 1000);
			uploadNow(c);
			return;
		}
		
		//get the times sent
		int timesSent = Config.getInt(c, TIMES_SENT);
		...
		if (timesSent > 1)
		{
			...
			TrackingDBHandler tdbh = new TrackingDBHandler(c);
			tdbh.writeLocation(LocationCode.NO_DATA, Util.currentTimeAdjusted() / 1000);
			uploadNow(c);
			return;
		}
		
		//get the actual location
		double latestLat = Config.getFloat(c, LATEST + LAT, 0);
        double latestLong = Config.getFloat(c, LATEST + LONG, 0);
        double latestAcc = Config.getFloat(c, LATEST + ACC, 0);
		
		//make sure we are in range
		int numLocs = 0; //Config.getSetting(c, Config.NUM_LOCATIONS_TRACKED, 0); //FIXME
		if (numLocs != 0)
		{
			log = false;
			for (int i = 0; i < numLocs; i++)
			{
				...
				//FIXME
				double thisLon = Config.getFloat(
						c, Config.TRACKED_LONG + i, (float) -1.0);
				double thisLat = Config.getFloat(
						c, Config.TRACKED_LAT + i, (float) -1.0);
				double thisRad = Config.getFloat(
			...
			if (log == false)
			{
				...
				TrackingDBHandler tdbh = new TrackingDBHandler(c);
				tdbh.writeLocation(LocationCode.OUT_OF_RANGE, Util.currentTimeAdjusted() / 1000);
				uploadNow(c);
				return;
			}
		}
	...
	//oddly, now that we are working with multiple studies, this is now necessary
	/**
